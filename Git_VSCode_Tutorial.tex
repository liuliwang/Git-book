...
\newtcolorbox{tipbox}[1][]{
  colback=tipgreen!5!white,
  colframe=tipgreen!70!black,
  fonttitle=\bfseries,
...
\newtcolorbox{warnbox}[1][]{
  colback=warnred!5!white,
  colframe=warnred!70!black,
  fonttitle=\bfseries,
...
\begin{center}
\begin{tikzpicture}[
  box/.style={draw, rounded corners=10pt, minimum width=3.5cm, minimum height=2.5cm,
              font=\small, align=center, fill=#1, inner sep=8pt},
  ...
  \node[box=green!15] (github) at (5,0) {
    \textbf{GitHub = 云端仓库}\\
    \faCloud~ 代码存在云上\\
    \faUsers~ 多人协作的中心枢纽\\
    \faGlobe~ 换电脑也能接着干
  };
  \node[box=orange!15] (vscode) at (2.5,-3) {
    \textbf{VS Code = 驾驶舱}\\
  ...
  \draw[arr] (git) -- node[above, font=\small]{推送/拉取} (github);
  \draw[arr] (vscode) -- node[left, font=\small]{图形化操作} (git);
  \draw[arr] (vscode) -- node[right, font=\small]{一键同步} (github);
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[
  phase/.style={draw, rounded corners=8pt, minimum width=4cm, minimum height=1.6cm,
                font=\small\bfseries, align=center, fill=#1},
  arr/.style={->, thick, >=stealth},
  node distance=1.8cm
]
  \node[phase=blue!20] (p0) at (0,0) {第 0 章\\建立心智模型\\\normalfont\small(5 分钟)};
  \node[phase=green!20] (p1) at (5,0) {第 1--3 章\\核心概念 + 基础操作\\\normalfont\small(2--3 小时)};
  \node[phase=yellow!20] (p2) at (10,0) {第 4--5 章\\分支协作 + 远程同步\\\normalfont\small(2--3 小时)};
  \node[phase=orange!20] (p3) at (15,0) {第 6--7 章\\VS Code 实战 + 完整流程\\\normalfont\small(1--2 小时)};
  \node[phase=red!20] (p4) at (7.5,-3) {附录\\避坑指南 + 命令速查\\\normalfont\small(随时查阅)};
...
\begin{enumerate}[leftmargin=1.5em]
  \item \textbf{分布式架构}：每个开发者的机器上都有一份完整的代码仓库和历史记录，
        无需联网也能进行提交、查看历史、创建分支等操作。
  \item \textbf{快照式存储}：Git 不是记录文件差异（diff），而是对每次提交保存整个项目
        的快照。未修改的文件只保留指向旧快照的链接，因此效率极高。
  \item \textbf{强大的分支}：Git 的分支操作几乎是瞬时的（仅创建一个指针），
        创建、切换、合并分支的成本极低，鼓励频繁使用分支。
  \item \textbf{数据完整性}：每次提交都通过 SHA-1 哈希校验，任何篡改都会被检测到。
  \item \textbf{暂存区设计}：独特的暂存区（Staging Area）机制，允许精确控制每次提交
        包含哪些变更。这是 Git 区别于其他版本控制系统的重要特性。
\end{enumerate}
\end{keybox}
...
\begin{tikzpicture}[
  box/.style={draw, rounded corners=8pt, minimum width=3.5cm, minimum height=1.2cm,
              font=\bfseries, align=center, fill=#1},
  arr/.style={->, thick, >=stealth},
  node distance=1.5cm
]
  \node[box=orange!20] (blob) {Blob\\{\small 文件内容}};
  \node[box=green!20, right=1.5cm of blob] (tree) {Tree\\{\small 目录结构}};
  \node[box=blue!20, right=1.5cm of tree] (commit) {Commit\\{\small 提交记录}};
  \node[box=purple!20, right=1.5cm of commit] (branch) {Branch\\{\small 分支指针}};
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  obj/.style={draw, rounded corners=5pt, minimum width=2.5cm, minimum height=0.8cm,
  ...
  % Blob nodes
  \node[obj, fill=orange!15] (blob1) at (-2.5,-4.5) {\textbf{Blob}\\main.py};
  \node[obj, fill=orange!15] (blob2) at (0,-4.5) {\textbf{Blob}\\README};
  \node[obj, fill=orange!15] (blob3) at (2.5,-4.5) {\textbf{Blob}\\.gitignore};

  % Parent commit
  \node[obj, fill=blue!15, minimum height=1cm] (parent) at (4,0) {
    \textbf{Parent Commit}\\
    abc123...
  };
...
\begin{infobox}
\textbf{Git vs GitHub 的区别}（初学者常混淆）：
...
这是学习 Git 最重要的一张图，请务必理解：

\begin{center}
\begin{tikzpicture}[
  box/.style={draw, rounded corners=8pt, minimum width=3cm, minimum height=1.5cm,
              font=\bfseries, align=center, fill=#1},
  arr/.style={->, thick, >=stealth},
  node distance=1.2cm
]
  \node[box=red!15] (work) {工作区\\(Working Directory)\\{\small 磁盘上的文件}};
  \node[box=yellow!15, right=2cm of work] (stage) {暂存区\\(Staging Area / Index)\\{\small 准备提交的变更}};
  \node[box=green!15, right=2cm of stage] (repo) {本地仓库\\(Repository)\\{\small 已提交的历史}};
  \node[box=blue!15, right=2cm of repo] (remote) {远程仓库\\(Remote)\\{\small GitHub 上的副本}};
...
VS Code 内置了强大的 Git 集成，将上述所有操作都图形化了。下图展示了 VS Code 源代码管理视图
的全貌——你将在本教程中反复看到这个界面：

\vscodeimg{images/01-sc-overview.png}{VS Code 源代码管理视图全景——左侧为变更文件列表，上方为提交信息输入框和提交按钮。这是你与 Git 交互的主要界面}
...
每个 Git 项目都始于一个\textbf{仓库}（Repository）。有两种方式创建仓库：
...
这不是一个 Git 仓库，并显示"初始化仓库"按钮：

\vscodeimg{images/12-init-repo.png}{VS Code 中的"初始化仓库"按钮——打开一个非 Git 项目时，源代码管理视图会提示你初始化}
...
\vscodeimg{images/11-clone-repo.png}{VS Code 的 Git: Clone 命令——按 Ctrl+Shift+P 输入"clone"，粘贴仓库 URL 即可克隆}
...
\begin{tikzpicture}[
  ...
  % Legend
  \node[anchor=west, font=\small] at (0,-3.5) {
...
在 VS Code 中，\texttt{git status} 的信息直接体现在源代码管理视图里：

\vscodeimg{images/13-modified-files.png}{修改后的文件列表——源代码管理视图的"更改"区域显示所有已修改但未暂存的文件，每个文件旁有状态标记}
...
\vscodeimg{images/05-stage-changes.png}{暂存操作——点击文件右侧的 "+" 按钮暂存单个文件，或点击"更改"标题旁的"+"按钮全部暂存}
...
\vscodeimg{images/15-commit-button.png}{提交操作——在顶部输入框填写提交信息，然后点击"提交"按钮（或按 Ctrl+Enter）完成提交}
...
在源代码管理视图中，\textbf{点击文件名}即可打开差异对比视图：

\vscodeimg{images/03-diff-editor.png}{差异对比编辑器——左侧为旧版本（HEAD），右侧为当前修改。绿色背景表示新增内容，红色背景表示删除内容}
...
\vscodeimgnarrow{images/24-diff-sidebyside.png}{并排视图——左右分栏，适合大幅修改}
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\textwidth}
\vscodeimgnarrow{images/25-diff-inline.png}{内联视图——变更在同一文件中显示，适合小改动}
\end{minipage}
\vspace{0.5cm}

\subsection{编辑器中的实时变更指示}

在代码编辑器中，VS Code 会在行号右侧的 gutter 区域\textbf{实时}显示变更指示：

\vscodeimg{images/21-gutter-diff.png}{编辑器 Gutter 变更指示器——绿色竖线表示新增行，蓝色竖线表示修改行，红色三角表示删除行。点击可查看详细信息}
...
\vscodeimg{images/18-current-branch.png}{底部状态栏的分支指示器——显示当前所在分支名称（如"main"），点击可切换或创建分支}

创建新分支时，在弹出的输入框中输入分支名称即可（如 \texttt{feature/login}），VS Code 会自动从当前分支创建并切换：

\vscodeimg{images/19-create-branch.png}{创建新分支——输入分支名称后，VS Code 会自动从当前分支创建并切换}
...
\vscodeimg{images/merge-conflict.png}{VS Code 中的合并冲突标记——文件中用特殊颜色标记冲突区域，并提供快捷操作按钮}
...
\vscodeimg{images/08-merge-editor.png}{VS Code 的三方合并编辑器——左侧为当前分支（Current），右侧为传入分支（Incoming），底部为合并结果}
...
\vscodeimg{images/17-pull-push.png}{Pull 和 Push 操作——在源代码管理视图的"..."菜单中可以单独执行拉取或推送}

\vscodeimg{images/16-sync-button.png}{同步按钮——源代码管理视图顶部的同步操作按钮，点击可执行一键同步（pull + push）}
...
\vscodeimg{images/13-modified-files.png}{VS Code 一键发布——对于未初始化的项目，VS Code 提供"初始化仓库"按钮，之后可以直接"发布到 GitHub"}
...
\vscodeimgwide{images/01-sc-overview.png}{源代码管理视图完整界面——各区域标注：(A) 提交信息输入框 (B) 提交按钮及下拉菜单 (C) 暂存的更改 (D) 更改列表 (E) 文件状态标记}
...
\vscodeimg{images/02-sc-graph.png}{源代码管理图形视图——以时间线形式展示提交历史，不同颜色区分不同分支，点击节点可查看详细信息}

\section{暂存（Stage）与提交（Commit）详解}

\subsection{暂存文件}

\vscodeimg{images/22-stage-detail.png}{暂存操作详情——每个文件右侧有"+"按钮可以单独暂存，"更改"标题旁有全部暂存按钮}

\subsection{部分暂存（Stage Selected Ranges）}

VS Code 支持精细到\textbf{代码行}级别的暂存——一个文件中改了多处，可以只暂存其中一部分：

\vscodeimg{images/23-stage-lines.png}{部分暂存——打开文件 Diff 后，可以选择特定的代码块进行暂存，而非整个文件}
...
\vscodeimg{images/07-sync-changes.png}{同步更改——底部状态栏显示当前分支和同步状态（↑ 待推送，↓ 待拉取），点击旋转箭头图标执行一键同步}

\section{分支管理详解}

\subsection{查看和切换分支}

当前分支名称显示在 VS Code \textbf{底部状态栏的左下角}，点击即可查看分支列表、切换分支或创建新分支。创建新分支时，VS Code 会自动从当前分支创建并立即切换：

\vscodeimg{images/19-create-branch.png}{创建新分支——输入新分支名称后，VS Code 会自动从当前分支创建并切换}

\section{Timeline 视图：文件历史}

VS Code 的 Timeline 视图（默认在资源管理器底部）显示当前文件的所有提交历史：

\vscodeimg{images/10-timeline.png}{Timeline 视图——显示当前文件的所有提交历史，按时间排列。点击任意提交可查看该次修改的详细内容，右键可选择"比较"或"还原"}
...
\vscodeimg{images/04-gutter-blame.png}{GitLens 的行内 Blame 注释——每行代码后显示灰色文字：最后修改者、时间和提交信息摘要}
...
\vscodeimg{images/20-view-changes.png}{开发中的 VS Code——编辑器 Gutter 实时显示你的修改（绿色/蓝色/红色标记），源代码管理视图列出所有变更文件}
...
\begin{tikzpicture}[
  zone/.style={draw, rounded corners=12pt, minimum width=3.2cm, minimum height=2.8cm,
               font=\small, align=center, fill=#1, inner sep=10pt},
  ...
  % Legend
  \node[anchor=north, font=\small, text=gray] at (7.5,-3.5) {
...
\begin{tikzpicture}[
  ...
  % ===== MERGE =====
  \node[anchor=south, font=\bfseries\large, text=blue!70!black] at (4,5) {Merge：保留完整历史，产生菱形合并节点};

  % main branch
  \node[commit=blue!60, tag] (m1) at (0,4) {A};
  \node[commit=blue!60, tag] (m2) at (2,4) {B};
  \node[commit=blue!60, tag] (m3) at (4,4) {C};
  \node[commit=red!60, tag] (m4) at (6,4) {M};
  \node[commit=blue!60, tag] (m5) at (8,4) {D};

  % feature branch
  \node[commit=green!60, tag] (f1) at (4,2.5) {E};
  \node[commit=green!60, tag] (f2) at (6,2.5) {F};
  ...
  % ===== REBASE =====
  \node[anchor=south, font=\bfseries\large, text=orange!70!black] at (4,-1) {Rebase：线性历史，无合并节点};

  % main branch (same)
  \node[commit=blue!60, tag] (rm1) at (0,-2) {A};
  \node[commit=blue!60, tag] (rm2) at (2,-2) {B};
  \node[commit=blue!60, tag] (rm3) at (4,-2) {C};

  % feature rebased onto main
  \node[commit=green!60, tag] (rf1) at (6,-2) {E'};
  \node[commit=green!60, tag] (rf2) at (8,-2) {F'};
...
\begin{tikzpicture}[
  box/.style={draw, rounded corners=8pt, minimum width=3.5cm, minimum height=1.6cm,
              font=\small, align=center, fill=#1, inner sep=6pt},
  arr/.style={->, thick, >=stealth},
  cmd/.style={font=\small\ttfamily, text=blue!70!black},
  node distance=1.8cm
]
  ...
  % Legend
  \node[anchor=north, font=\small, text=gray] at (6,-4.5) {
