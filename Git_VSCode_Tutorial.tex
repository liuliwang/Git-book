...
Git、GitHub 和 VS Code 这三个名字经常一起出现，但它们各自扮演的角色其实截然不同。对初学者来说，最大的困惑往往是"到底该用哪个工具做什么"。直接记命令容易忘，不如先在大脑里建立三个鲜活的画面。我们把 Git 想象成一台时光机，它负责记录你代码的每一次变化，让你随时能回到过去。GitHub 则像一座放在云端的仓库，专门帮你把代码备份到网上，同时让多人协作变得简单。VS Code 是驾驶舱，它把 Git 和 GitHub 的能力包装成按钮和菜单，让你不用记命令也能完成操作。三者关系如图~\ref{fig:three-analogies} 所示——图中三条箭头说清了它们的分工：Git 负责保存历史，GitHub 负责云端同步与协作，VS Code 负责把这一切翻译成图形界面，让你点按钮就能完成操作。
...
玩过单机游戏的人对"存档"一定不陌生。平时随便存档不心疼，关键 boss 战前存一个，打崩了就读档重来。Git 的每次提交（commit）就像给整个项目拍了一张照片，照片会被完整保存下来。想回到过去任何一刻都可以，而且"读档"不会弄坏你现在的工作，因为 Git 的读档不会动到你当前的工作区。如果你突然想尝试一个新想法，可以开一条新的剧情线，这就是分支，几条线互不影响，搞砸了删掉重来就行。
...
\medskip
\noindent\textbf{提示：}零基础心法——别怕弄坏！Git 设计的初衷就是让你「大胆试错，随时后悔」。
\medskip

\noindent\textbf{专业映射：}「存档」在 Git 里叫\textbf{提交（commit）}，每次提交记录的是整个文件夹在那一刻的完整样子——不是只记"改了哪几行"。「时光倒流」对应 Git 的回到历史版本功能，「平行宇宙」对应分支：分支只是一个小标签，指向某一次存档，想开一个就开一个，几乎不花任何成本。详见第 1 章与第 4 章。
\medskip

\subsection{GitHub 是「云端仓库」}

代码放在自己电脑上，硬盘坏了或电脑丢了就全没了。GitHub 不是普通网盘，它不只是存文件，还记录谁改了什么、怎么合并、如何讨论。你可以把它理解成一个大家共同使用的「公共驿站」：你把自己的改动推上去，别人就能拉下来，所有人都能随时拿到最新版本。
...
\medskip
\noindent\textbf{专业映射：}Git 在设计上并没有「中心服务器」这个概念——每台电脑上的仓库地位是平等的，都能完整保存全部历史。GitHub 之所以显得像「云端仓库」，只是因为大家约定俗成，把它作为共同使用的集散地。详见第 5 章。
\medskip

\subsection{VS Code 是「驾驶舱」}

汽车有仪表盘也有引擎盖——你开车不需要懂发动机原理，但仪表盘上的每个灯背后都有真实机械在运行。VS Code 就是 Git 的仪表盘：它把 Git 的复杂命令翻译成按钮、菜单、颜色竖线，你点按钮时，背后其实是一条条 Git 命令在替你执行。那些颜色竖线（绿色新增、蓝色修改、红色删除）就像仪表盘上的指示灯，随时告诉你引擎（Git）正在做什么。
...
\medskip
\noindent\textbf{专业映射：}你在 VS Code 里点的每个按钮，背后都对应一条 Git 命令——点「提交」，就是替你执行了 \texttt{git commit}；点「推送」，就是替你执行了 \texttt{git push}。界面和命令只是同一件事的两种表达方式：学会看 VS Code 的输出，就能逐渐看懂命令行；反过来，懂一点命令行，也能更清楚 VS Code 在做什么。详见第 6 章。
\medskip

\section{Git 的设计哲学：四个关键词}

类比帮你"想象"，但要真正理解 Git，需要四个关键词。每个关键词背后都对应 Git 设计中的一条根本取舍；本节只点出本质，细节留待第 1 章及后续章节展开。

\subsection{关键词一：每个存档都有独一无二的编号}

Git 给每次提交生成一个独一无二的编号，这个编号由文件内容本身算出来：内容一样编号就一样，哪怕只改一个标点，编号也会完全变。所以只要编号对得上，就能确信文件没有被悄悄改过。

\subsection{关键词二：保存完整快照，不是只记差异}

Git 每次提交保存的不是"改了哪几行"，而是整个项目在那一刻的完整样子。听起来很占空间，其实没变过的文件不会重复保存；需要看"改了什么"时，Git 再对比两个完整样子算给你看。

\subsection{关键词三：历史是一张网，不是一条直线}

每次提交都记着自己的"前一个提交"是谁。多人并行开发时，历史会分成好几条线，合并时再汇到一起——所以 Git 的历史是一张网，不是一条直线。正因为这样，多人同时开发互不干扰、合并也顺理成章。

\subsection{关键词四：分支和标签只是小纸条}

分支、标签在 Git 里只是一个个小标签，写着"我指向哪一次提交"。创建分支就是贴一个新标签，几乎不花任何成本——所以 Git 鼓励你多开分支、大胆尝试。

\medskip
\noindent\textbf{小结：}编号保证数据可靠、完整保存保证历史清晰、网状历史保证并行开发、小标签保证操作轻快——这四个设计加起来，让 Git 成为版本控制的事实标准。版本控制系统的三代演进（RCS $\to$ CVS/SVN $\to$ Git）详见\textbf{第 1 章表~\ref{tab:vc-history}}，BitKeeper 事件的来龙去脉见第 1 章正文。
\medskip

\section{本教程的学习路径图}

教程共 10 章加附录，按认知规律四层递进：先用三个类比建立直观画面（第 0 章），再系统掌握 Git 的核心概念与基础命令（第 1--3 章），学成分支协作与远程同步，开启多人开发的真正威力（第 4--5 章），最后回到 VS Code 实战和完整工作流，让日常操作高效顺畅（第 6--7 章）。附录（第 8--9 章）跨阶段覆盖：避坑指南帮你躲开最常见的坑，命令速查卡供你随时翻阅。完整路径如图~\ref{fig:learning-path} 所示——
\begin{figure}[H]\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
...
在开始正式学习之前，花两分钟确认工具箱就绪。下面四步全部通过，说明 Git 和 VS Code 都已正确安装。
\medskip
...
\textbf{全部通过？环境已就绪，直接翻页进入第 1 章。}\\
\textbf{某一步失败？} 别着急——翻到第 2 章跟着安装步骤走，装完再回到这里继续；整个过程不超过 10 分钟。
...
\caption{版本控制的发展简史}\label{tab:vc-history}

{\renewcommand{\arraystretch}{1.3}\setlength{\tabcolsep}{6pt}\begin{tabularx}{\textwidth}{l l l >{\raggedright\arraybackslash}X}
\toprule
...
\begin{table}[H]\centering
\caption{集中式 vs 分布式版本控制}\label{tab:centralized-vs-distributed}
{\renewcommand{\arraystretch}{1.3}\setlength{\tabcolsep}{6pt}\begin{tabularx}{\textwidth}{l >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
\toprule
...
\medskip
\noindent\textbf{提示：}Porcelain vs Plumbing——Git 命令分为两层\textbf{表层（Porcelain）}是你日常使用的命令（\texttt{git add}、\texttt{git commit}、\texttt{git log}），设计为人类友好；\textbf{底层（Plumbing）}是 Git 内部使用的命令（\texttt{git cat-file}、\texttt{git hash-object}、\texttt{git ls-tree}），输出格式稳定，适合脚本调用。理解数据模型时，底层命令是最好的"显微镜"。
...
\begin{table}[H]\centering
\caption{Git vs GitHub 的区别}\label{tab:git-vs-github}
...
\medskip
\noindent\textbf{提示：}类比理解——把 Git 想象成拍照。
\begin{itemize}
...
\medskip
\noindent\textbf{提示：}成功标志——打开 Windows 资源管理器进入 \texttt{my-first-repo} 文件夹，点击菜单栏「查看」$\to$ 勾选「隐藏的项目」，可以看到 \texttt{.git} 文件夹。
...
\medskip
\noindent\textbf{提示：}日常使用中，绝大多数配置使用 \texttt{--global} 即可。如果某个项目需要特殊配置
...
\medskip
\noindent\textbf{提示：}VS Code 首次连接 GitHub 时，会自动弹出登录提示，引导你通过浏览器完成 OAuth 认证。
...
\medskip
\noindent\textbf{提示：}成功标志——SSH 密钥页面出现名为 \texttt{My Laptop} 的密钥记录；
...
\medskip
\noindent\textbf{提示：}\texttt{git status} 是你最应该频繁执行的命令。它告诉你当前处于什么状态，
...
\medskip
\noindent\textbf{提示：}VS Code 支持 AI 辅助生成提交信息——点击提交信息输入框旁的"魔法棒"图标，
...
\medskip
\noindent\textbf{提示：}在 VS Code 中，取消暂存就是点击"暂存的更改"区域中文件旁边的 "-" 按钮。
\medskip
...
\noindent\textbf{提示：}\textbf{成功标志}：点击源代码管理视图标题栏的「提交历史图」图标，能看到一条提交记录 \texttt{更新问候语}。
\medskip
...
\noindent\textbf{提示：}\textbf{搜索冲突}：按 \texttt{Ctrl + Shift + P}，输入 "Merge Conflict"，
可以看到所有冲突相关的命令，包括"Accept All Current"等批量操作。

\subsection{远程协作中的冲突（PR 冲突）}

前面讲的是本地合并冲突。但在团队协作中，还有一种更常见的场景：
你创建了一个 Pull Request（PR），继续在该分支上工作并再次 push 后，
PR 页面突然提示"This branch has conflicts that must be resolved"。
这不是你的 push 失败了，而是远程仓库的历史发生了分叉。

\subsubsection{PR 冲突的产生原因}

想象两条铁路从同一个车站（共同祖先）出发，分别向不同方向延伸。
一条是 \texttt{main} 分支，另一条是你的 \texttt{feature} 分支。
当你创建 PR 后，\texttt{main} 上可能又有其他人提交了新的代码，
而你的 \texttt{feature} 分支也继续提交了新的修改。
如果两边的修改恰好落在同一文件的同一区域，Git 就无法自动合并，PR 页面就会提示冲突。

用时间线表示，冲突的产生过程如下：

\begin{verbatim}
main:    A --- B --- C --- D          <- PR 后 main 新增 C、D
feature: A --- B --- E --- F          <- 分支上继续提交 E、F
               |
               | 双方在 B 后分叉，改到同一处 -> PR 冲突
\end{verbatim}

\begin{infobox}
PR 冲突的本质和本地冲突一样：两个分支修改了同一文件的同一部分，Git 不知道听谁的。
区别在于，本地冲突发生在你执行 \texttt{git merge} 时，而 PR 冲突发生在远程仓库尝试合并时。
\end{infobox}

\subsubsection{PR 冲突与本地冲突的区别}

\begin{itemize}[leftmargin=2em]
  \item \textbf{发生位置不同}：本地冲突在你的电脑上；PR 冲突在远程仓库（GitHub/Gitee 服务器端）。
  \item \textbf{触发时机不同}：本地冲突是执行 \texttt{git merge} 时触发；PR 冲突是 PR 页面尝试合并时检测到的。
  \item \textbf{解决方式不同}：本地冲突直接在本地解决后提交；PR 冲突需要先同步远程最新代码，解决后再 push 更新 PR。
\end{itemize}

\begin{keybox}
PR 冲突不是你的 push 失败了，而是 PR 双方的历史分叉所致。
只要将远程 \texttt{main} 的最新内容同步到你的分支，解决冲突后再 push，PR 就会恢复正常。
\end{keybox}

\subsubsection{解决 PR 冲突的方法}

解决 PR 冲突有两种方式：命令行和 VS Code 图形界面。
...
# 2. 方式一：用 rebase 把本地提交变基到最新 main 上（推荐，历史整洁）
git rebase origin/main
# 解决冲突后：git add <文件> 再 git rebase --continue
# 最后强制推送更新 PR：git push --force-with-lease

# 2. 方式二：用 merge 合并远程 main 到本地分支
git merge origin/main
# 解决冲突后：git add <文件> 再 git commit
# 最后推送更新 PR：git push
\end{lstlisting}

\begin{warnbox}
\textbf{rebase 会重写提交历史}。如果分支已推送到远程且被他人使用，不要 rebase，改用 merge；
否则会导致他人的本地分支与远程不一致。
\end{warnbox}

\begin{tipbox}
\textbf{--force-with-lease} 比 \texttt{--force} 更安全：
它只在远程没有新提交时才强制覆盖，避免覆盖他人的工作。
\end{tipbox}

\textbf{方式二：VS Code 图形界面}

在 VS Code 中解决 PR 冲突的步骤如下：

\begin{enumerate}[leftmargin=1.5em]
  \item 点击源代码管理视图顶部的"同步"按钮（或执行 \texttt{git fetch origin}），获取远程最新代码。
  \item 执行 rebase 或 merge 操作后，VS Code 会显示冲突标记和快捷操作按钮（如图~\ref{fig:merge-conflict} 所示）。
  \item 使用"Accept Current Change"、"Accept Incoming Change"等按钮，或手动编辑冲突区域。
  \item 删除所有冲突标记（\texttt{<<<<<<<}、\texttt{=======}、\texttt{>>>>>>>}），保存文件。
  \item 在源代码管理视图中，点击冲突文件旁的"+"暂存（标记为已解决）。
  \item 输入提交信息，点击"提交"完成合并。
  \item 最后 push 更新 PR。
\end{enumerate}
...
\begin{infobox}
PR 冲突不是分支坏了，而是双方改了同一处代码，解决后推送即可。
核心思路是：先同步远程最新代码，解决冲突，再推送更新 PR。
\end{infobox}

\subsubsection{预防措施}

\begin{enumerate}[leftmargin=1.5em]
  \item \textbf{经常同步 main}：定期执行 \texttt{git fetch origin}，将 \texttt{main} 的最新内容同步进自己的分支（通过 rebase 或 merge）。
  \item \textbf{小步快跑}：提交要小、PR 要小，冲突面就小。一个 PR 只解决一个问题，避免大段代码长时间不合并。
  \item \textbf{合并前再确认}：PR 合并前再次同步 \texttt{main}，确认无冲突后再点"Merge"。
  \item \textbf{提前沟通}：大改动前先和团队沟通改动范围，避免多人同时改同一文件。
\end{enumerate}
...
\noindent\textbf{提示：}\textbf{成功标志}：底部状态栏分支名为 \texttt{main}，源代码管理视图无待处理更改。
...
\noindent\textbf{提示：}\textbf{好的 PR 描述应包含}：
\begin{itemize}
  \item 这个 PR 解决了什么问题？
  \item 做了什么改动？
  \item 如何测试这些改动？
...
\noindent\textbf{提示：}\textbf{成功标志}：GitHub 仓库页面的 \texttt{main} 分支中出现了 \texttt{README.md}，Pull Request 状态显示为 Merged（已合并）。
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{Gutter Indicators}：编辑器行号旁的绿/蓝/红标记快速显示哪些行有改动（详见第~\ref{sec:diff-editor}~节）。
  \item \textbf{AI 提交信息}：提交信息框右侧的星标图标可一键生成提交信息（详见第~\ref{sec:ai-git}~节）。
  \item \textbf{Code Review 按钮}：暂存更改后可点击 Code Review 让 AI 审查（详见第~\ref{sec:ai-git}~节）。
\end{itemize}

\begin{tipbox}
\textbf{快速预览 vs 详细阅读}

上述三项功能在 Source Control 视图中都有入口，但完整用法和配置在第~\ref{sec:diff-editor}~节（Diff 编辑器）和第~\ref{sec:ai-git}~节（AI 辅助功能）中有更详细的介绍。建议先记住入口位置，需要时再深入阅读。
\end{tipbox}
...
打开文件的 Diff 视图后，编辑器 gutter 上会出现 \textbf{Stage} 和 \textbf{Revert} 按钮，可以直接从 diff 视图暂存或还原单个代码块，无需回到 Source Control 侧边栏。此外，你也可以把文件从"更改"列表直接拖到"已暂存的更改"列表实现暂存。详细操作见第~\ref{sec:diff-editor}~节。

\begin{tipbox}
\textbf{拖动暂存的小技巧}

在 Source Control 视图中，按住文件直接拖动到"已暂存的更改"区域即可完成暂存。这种方式特别适合需要快速挑选多个文件的场景，比逐个点按"+"按钮更高效。
\end{tipbox}

\section{Diff 编辑器详解（Diff Editor）}\label{sec:diff-editor}

Diff 编辑器是审查代码修改的核心工具。在 VS Code 中，
它把文件的原始版本和当前修改并排展示，让你一眼看清
每一处改动。

\textbf{打开 Diff 编辑器的两种方式}：

\begin{itemize}[leftmargin=2em]
  \item \textbf{点击文件名}：在 Source Control 视图的"更改"
        或"暂存的更改"列表中，直接点击任意文件名。
  \item \textbf{命令面板}：按 \texttt{Ctrl + Shift + P}，输入
        "Git: Open Changes"，选择当前文件。
\end{itemize}

\subsection{并排视图与内联视图}

Diff 编辑器默认使用\textbf{并排视图}（Side-by-Side），
左侧显示文件的原始版本，右侧显示当前修改。
绿色背景表示新增内容，红色背景表示删除内容，如图~\ref{fig:diff-editor-sidebyside} 所示。

\vscodeimg{images/35-diff-editor-sidebyside.png}{Diff 编辑器并排视图——左侧为原始版本，右侧为当前修改，绿色表示新增，红色表示删除}{fig:diff-editor-sidebyside}

如果屏幕较窄或改动较小，可以切换到\textbf{内联视图}
（Inline View）。点击 Diff 编辑器右上角工具栏的
"..."（More Actions）菜单，选择 \textbf{Inline View} 即可，内联视图如图~\ref{fig:diff-editor-inline} 所示。

\vscodeimg{images/36-diff-editor-inline.png}{Diff 编辑器内联视图——变更在同一文件中紧凑显示，适合小改动或窄屏}{fig:diff-editor-inline}

\begin{tipbox}
\textbf{视图切换快捷键}

内联视图和并排视图没有默认快捷键，但你可以通过
命令面板（\texttt{Ctrl + Shift + P}）搜索
"Toggle Inline View"快速切换。
\end{tipbox}

\subsection{常用操作}

Diff 编辑器提供了多种高效操作，帮助你快速审阅修改：

\begin{enumerate}[leftmargin=1.5em]
  \item \textbf{Collapse Unchanged Regions}：折叠未更改区域。
        对于大文件，这能让你专注于实际修改的部分。
        点击编辑器右上角的折叠图标即可。
  \item \textbf{Next Change / Previous Change}：导航按钮。
        点击工具栏的左右箭头，快速跳转到上一个或
        下一个变更点，无需手动滚动。
  \item \textbf{Stage / Revert}：Diff 编辑器 gutter 中的
        直接操作。每个变更块旁边会出现 "+"（暂存）和
        "-"（还原）按钮，点击即可对单个代码块执行操作，
        无需回到 Source Control 视图。
\end{enumerate}

\begin{warnbox}
\textbf{从 Diff 视图直接暂存}

Diff 编辑器 gutter 中的 Stage 按钮是最精细的暂存方式——
它可以只暂存一个代码块，而"部分暂存"（Stage Selected Ranges）
甚至可以精确到行。这比暂存整个文件更灵活。
\end{warnbox}

此外，你还可以\textbf{拖动文件进行暂存}：在 Source Control
视图中，直接把"更改"列表中的文件拖到"暂存的更改"区域，
文件即被暂存。这种方式直观且高效。

\subsection{Gutter Indicators（行号旁的状态标记）}

除了 Diff 编辑器，VS Code 还在普通编辑器的\textbf{行号旁}
（gutter 区域）实时显示变更指示。这些标记让你在不打开
Diff 视图的情况下，也能快速了解当前文件的修改分布，如图~\ref{fig:gutter-indicators} 所示。

\vscodeimg{images/37-gutter-indicators.png}{编辑器 Gutter Indicators——绿色竖条表示新增行，蓝色竖条表示修改行，红色三角表示删除行}{fig:gutter-indicators}

\textbf{三种标记的含义}：

\begin{itemize}[leftmargin=2em]
  \item \textbf{绿色竖条}：新增行（Added）
  \item \textbf{蓝色竖条}：修改行（Modified）
  \item \textbf{红色三角}：删除行（Deleted），显示在删除点上方
\end{itemize}

这些标记不仅出现在编辑器左侧的 gutter 中，在右侧的
\textbf{minimap}（代码缩略图）中也同样可见，方便你快速
定位到文件中的修改位置。

点击任意 gutter 指示器，会展开一个\textbf{内联 diff 预览}，
显示该处修改的详细内容。预览中同样提供 Stage 和 Revert
按钮，可以直接从预览中暂存或还原单个代码块，如图~\ref{fig:gutter-hover} 所示。

\vscodeimg{images/38-gutter-hover.png}{点击 Gutter Indicator 展开内联 diff 预览——可直接从预览中暂存或还原代码块}{fig:gutter-hover}

\textbf{相关设置}：在设置中搜索 \texttt{scm.diffDecorations}，
可以控制变更指示的显示范围：

\begin{itemize}[leftmargin=2em]
  \item \texttt{all}：全部显示（默认）
  \item \texttt{gutter}：仅显示在 gutter 中
  \item \texttt{overview}：仅显示在 overview 标尺中
  \item \texttt{minimap}：仅显示在 minimap 中
  \item \texttt{none}：完全隐藏
\end{itemize}

另一个相关设置是 \texttt{scm.diffDecorationsGutterVisibility}，
控制 gutter 指示器的显示时机：

\begin{itemize}[leftmargin=2em]
  \item \texttt{always}：始终显示
  \item \texttt{onHover}：鼠标悬停时显示
\end{itemize}

\begin{keybox}
\textbf{Diff 编辑器与 Gutter Indicators 的关系}

Diff 编辑器用于\textbf{详细审查}——看清单个文件的所有改动；
Gutter Indicators 用于\textbf{快速定位}——在编辑代码时一眼看到
修改分布。两者互补，日常开发中配合使用效率最高。
\end{keybox}
...
\section{AI 辅助功能（AI-Powered Git）}\label{sec:ai-git}

VS Code 从 1.90 版本起，在 Source Control 视图中集成了 AI 辅助功能。这些功能依托 GitHub Copilot 扩展，能够基于你的代码更改自动生成提交信息、审查修改内容，甚至辅助解决合并冲突。对于每天需要频繁提交和审查代码的开发者来说，AI 辅助可以显著减少重复劳动，让你把精力集中在代码本身。

本章介绍的 AI 功能均需要安装并登录 GitHub Copilot 扩展（部分功能需要有效订阅）。如果你尚未配置 Copilot，请先前往 VS Code 扩展市场安装 "GitHub Copilot" 扩展并登录 GitHub 账号。

\subsection{AI 生成提交信息}

写提交信息（Commit Message）是开发流程中不可避免但又容易敷衍的环节。好的提交信息应该清晰描述"做了什么"和"为什么做"，但手动撰写往往耗时且容易写成 "fix bug" 或 "update" 这类无意义的描述。

VS Code 的 AI 提交信息功能解决了这个问题。当你在 Source Control 视图中暂存了更改后，提交信息输入框右侧会出现一个星标（Sparkle）图标。点击该图标，Copilot 会分析当前暂存的代码差异，自动生成一段结构化的提交信息，如图~\ref{fig:ai-commit-message} 所示。

\vscodeimg{images/39-ai-commit-message.png}{AI 生成提交信息——Source Control 视图提交框右侧的星标图标，点击后基于暂存更改自动生成提交信息}{fig:ai-commit-message}

\textbf{使用步骤}：
\medskip
\begin{enumerate}[leftmargin=1.5em]
  \item 在 Source Control 视图中，将需要提交的更改暂存（Stage）。
  \item 点击提交信息输入框右侧的星标图标（Sparkle）。
  \item Copilot 分析暂存文件的 diff，自动生成提交信息草稿。
  \item 你可以直接接受、修改或重新生成。
\end{enumerate}
\medskip

\textbf{自定义生成风格}：

Copilot 支持通过 Custom Instructions 自定义提交信息的生成风格。在 VS Code 设置中搜索 "GitHub Copilot Commit Message Generation"，你可以添加类似以下的指令：
...
\begin{itemize}[leftmargin=2em]
  \item \texttt{git.useEditorAsCommitInput}：启用后，提交信息输入框会打开一个完整编辑器，方便撰写多段落提交信息（适合需要详细说明的提交）。
  \item \texttt{git.addAICoAuthor}：启用后，AI 生成提交信息时会自动添加 \texttt{Co-authored-by} 标记，记录 Copilot 的辅助贡献。
\end{itemize}

\begin{tipbox}
\textbf{AI 生成提交信息的最佳实践}

AI 生成的提交信息虽然省时，但建议养成"生成后检查"的习惯。重点关注：
\begin{itemize}[leftmargin=2em]
  \item 是否准确描述了改动的核心内容？
  \item 是否遗漏了重要的破坏性变更（Breaking Change）？
  \item 是否符合团队的提交信息规范（如 Conventional Commits）？
\end{itemize}
\end{tipbox}

\subsection{AI 代码审查（Code Review）}

除了生成提交信息，Copilot 还能在提交前帮你审查代码。在 Source Control 视图中，暂存更改后会出现一个 "Code Review" 按钮。点击后，Copilot 会分析当前更改并给出审查评论和改进建议，如图~\ref{fig:ai-code-review} 所示。

\vscodeimg{images/40-ai-code-review.png}{AI 代码审查——Source Control 视图中的 Code Review 按钮，AI 分析当前更改并给出审查评论}{fig:ai-code-review}

\textbf{Code Review 能做什么}：

\begin{itemize}[leftmargin=2em]
  \item \textbf{发现潜在问题}：如未处理的异常、边界条件遗漏、性能隐患等。
  \item \textbf{代码风格建议}：指出不符合团队规范或最佳实践的写法。
  \item \textbf{安全风险提示}：标记可能的 SQL 注入、XSS 等安全问题。
  \item \textbf{改进建议}：提供更简洁或更高效的实现方式。
\end{itemize}

\textbf{GitHub Pull Requests 中的 Code Review}：

如果你使用 GitHub Pull Requests 扩展，在 PR 的描述页面同样可以找到 Code Review 按钮。这里的审查范围更大，可以分析整个 PR 的所有更改，而不仅仅是当前暂存的文件。审查结果会以评论形式出现在 PR 中，方便团队成员查看和讨论。

\begin{warnbox}
\textbf{AI 审查的局限性}

AI 代码审查是辅助工具，不能替代人工审查。它可能：
\begin{itemize}[leftmargin=2em]
  \item 误报——把正确的代码标记为有问题（False Positive）。
  \item 漏报——遗漏真正重要的安全或逻辑缺陷。
  \item 建议不当——在某些场景下给出不符合业务需求的修改建议。
\end{itemize}
建议将 AI 审查作为第一道过滤，关键代码仍需人工复核。
\end{warnbox}

\subsection{AI 辅助解决合并冲突}

在第~\ref{sec:merge-conflicts}~节中，我们介绍了 VS Code 的三路合并编辑器和内联冲突解决工具。在较新版本的 VS Code 中，Copilot 还提供了 AI 辅助的合并冲突解决功能。

当遇到合并冲突时，Copilot 可以分析冲突双方的代码，自动生成一个"智能合并建议"。这个建议会尝试理解双方的修改意图，给出一个兼顾两边的合并结果。你可以在合并编辑器中看到 AI 建议的选项，选择接受、修改或忽略。

\begin{keybox}
\textbf{AI 辅助冲突解决的使用场景}

AI 辅助冲突解决特别适合以下情况：
\begin{itemize}[leftmargin=2em]
  \item 冲突双方只是格式或变量名的微小差异，AI 可以快速合并。
  \item 冲突涉及大量重复代码，手动合并容易出错。
  \item 你对某段代码不熟悉，不确定应该保留哪一方的实现。
\end{itemize}
但对于涉及核心业务逻辑的复杂冲突，仍建议手动审查每一行更改。
\end{keybox}

\begin{warnbox}
\textbf{Copilot 订阅要求}

本章介绍的 AI 辅助功能大部分需要有效的 GitHub Copilot 订阅。免费版 Copilot 有使用次数限制，部分高级功能（如 PR 级别的 Code Review）仅对付费订阅用户开放。具体功能和定价请参考 GitHub 官方页面。
\end{warnbox}
...
当前分支名称显示在 VS Code \textbf{底部状态栏的左下角}，点击即可查看分支列表、切换分支或创建新分支。创建新分支时，VS Code 会自动从当前分支创建并立即切换，如图~\ref{fig:create-branch-alt} 所示。如果需要在同一仓库中并行维护多个工作目录，分别处理不同的分支，可以进一步了解第~\ref{sec:git-worktrees}~节的工作树（Worktrees）功能。

\vscodeimg{images/19-create-branch.png}{创建新分支——输入新分支名称后，VS Code 会自动从当前分支创建并切换}{fig:create-branch-alt}

\section{Git Worktrees（工作树）}\label{sec:git-worktrees}

Git Worktree（工作树）是 Git 的一项高级功能，允许你在同一仓库中维护多个独立的工作目录。每个工作树可以检出不同的分支，拥有独立的文件系统视图，彼此之间互不干扰。这意味着你可以同时打开多个 VS Code 窗口，分别处理不同的功能分支，而无需频繁切换分支或暂存当前的工作进度。

在 VS Code 中，工作树与 Source Control 面板深度集成，你可以像管理普通仓库一样管理工作树，包括创建、打开、比较差异等操作。
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{并行开发}：同时处理多个功能分支，无需频繁 \texttt{git stash} 和 \texttt{git checkout}。
  \item \textbf{代码审查}：测试队友的 PR 时，无需暂存自己未提交的工作。
  \item \textbf{紧急修复}：生产环境出现 bug 时，可以在独立工作树中快速修复并发布，同时保持主工作区中的重构工作不受影响。
  \item \textbf{运行不同版本}：在不同工作树中运行同一应用的不同版本，方便对比行为差异。
\end{itemize}
...
在 VS Code 中创建工作树非常直观：

\begin{enumerate}[leftmargin=1.5em]
  \item 打开 Source Control 视图，在顶部的 Repositories 视图中选中目标仓库。
  \item 点击仓库名称右侧的 \textbf{More Actions(...)}（三点菜单）。
  \item 选择 \textbf{Worktrees} > \textbf{Create Worktree}。
  \item 在弹出的对话框中选择要检出的分支（或新建分支），并指定工作树的存放路径。
  \item VS Code 会自动创建文件夹并检出分支，新工作树随即出现在 Repositories 视图中，整个过程如图~\ref{fig:worktree-create} 所示。
\end{enumerate}

\vscodeimg{images/41-worktree-create.png}{创建工作树——通过 More Actions 菜单选择 Create Worktree，指定分支和路径}{fig:worktree-create}

创建完成后，你可以在 Repositories 视图中看到新出现的工作树条目，它与原仓库并列显示，方便你快速识别和管理，如图~\ref{fig:worktree-repositories} 所示。

\vscodeimg{images/42-worktree-repositories.png}{Repositories 视图中的工作树——新工作树作为独立条目显示，与原仓库并列}{fig:worktree-repositories}

\textbf{命令行方式}：

如果你更习惯使用命令行，Git 也提供了简洁的工作树命令：
...
# 基于远程分支创建工作树
git worktree add ../hotfix origin/main -b hotfix-branch
...
\textbf{打开工作树}：在 Repositories 视图中右键点击工作树条目，选择 \textbf{Open Worktree in New Window} 在新窗口中打开，或选择 \textbf{Open Worktree in Current Window} 在当前窗口中切换。

\textbf{比较差异}：使用 \textbf{Compare with Workspace} 命令，可以快速比较当前工作区与工作树之间的文件差异，方便审查和合并。

\textbf{迁移更改}：VS Code 提供了 \textbf{Migrate Worktree Changes} 命令，可以将工作树中的未提交更改合并回当前工作区，无需手动复制文件。

\textbf{自动检测}：默认情况下，VS Code 不会自动检测已有的工作树。你可以在设置中启用 \texttt{git.detectWorktrees}，让 VS Code 自动识别并显示仓库关联的所有工作树。

\textbf{文件复制配置}：通过设置 \texttt{git.worktreeIncludeFiles}，可以指定在工作树创建时自动复制的文件列表（如 \texttt{.gitignore}、\texttt{.env} 等），避免重复配置。注意，像 \texttt{node\_modules} 这样的大型依赖目录通常不应复制，而是通过包管理器重新安装。
...
工作树虽然强大，但也需要注意以下几点：
\begin{itemize}[leftmargin=2em]
  \item 工作树之间共享同一个 \texttt{.git} 目录，因此提交历史是共通的。在一个工作树中提交后，其他工作树立即可见。
  \item 不要在同一个工作树中再次运行 \texttt{git init}，这会破坏仓库结构。
  \item 删除工作树前，确保其中的更改已提交或迁移，否则可能丢失工作进度。
  \item 工作树路径最好与主仓库保持同级目录结构，方便管理和识别。
\end{itemize}
\end{tipbox}
...
\section{合并冲突解决（Merge Conflicts）}\label{sec:merge-conflicts}

当你把两个分支合并到一起时，如果它们对同一处代码做了不同的修改，Git 就无法自动判断该保留哪一方的改动，这种情况就叫\textbf{合并冲突}（Merge Conflict）。

冲突通常发生在以下场景：
\begin{itemize}[leftmargin=2em]
  \item 两个分支修改了同一文件的同一行代码，执行 \texttt{git merge} 时触发
  \item 从远程仓库 \texttt{git pull} 时，本地修改与远程新提交发生冲突
  \item 执行 \texttt{git rebase} 或 \texttt{git cherry-pick} 时也可能遇到
\end{itemize}

遇到冲突不要慌。VS Code 提供了两种可视化工具：三路合并编辑器和内联冲突解决。下面分别介绍。

\begin{warnbox}
\textbf{冲突的本质}

冲突不是错误，而是 Git 在向你求助："这里有两份不同的修改，请你来决定最终版本。"只要理解这一点，解决冲突就会变得从容。
\end{warnbox}

\subsection{三路合并编辑器（3-Way Merge Editor）}

VS Code 的三路合并编辑器是处理复杂冲突的利器。它把冲突拆成三个面板，让你一目了然，如图~\ref{fig:merge-editor-3way} 所示。

\vscodeimg{images/33-merge-editor-3way.png}{三路合并编辑器界面——左侧 Incoming 显示被合并分支的更改，右侧 Current 显示当前分支，底部 Result 为合并结果}{fig:merge-editor-3way}

\textbf{三面板布局}：

\begin{itemize}[leftmargin=2em]
  \item \textbf{Incoming（左）}：被合并分支（如 \texttt{feature}）的更改
  \item \textbf{Current（右）}：当前分支（如 \texttt{main}）的内容
  \item \textbf{Result（底部）}：合并后的结果，你可以在这里手动编辑
\end{itemize}

\textbf{解决冲突的操作步骤}：

\begin{enumerate}[leftmargin=1.5em]
  \item 打开冲突文件后，VS Code 自动进入三路合并视图。
  \item 每个冲突项旁边会出现复选框，勾选你想保留的更改（可单选或多选）。
  \item 在 Result 面板中手动微调，确保代码逻辑正确。
  \item 点击右上角的 \textbf{Complete Merge} 按钮，暂存并关闭编辑器。
\end{enumerate}

\textbf{进阶功能}：

\begin{itemize}[leftmargin=2em]
  \item 点击右上角的三点菜单，可切换为垂直布局，或显示 \textbf{Base} 视图（文件合并前的原始状态）。
  \item 编辑器顶部会显示未解决冲突计数指示器，实时告诉你还剩几处冲突。
  \item 如果不喜欢三路编辑器，可以在设置中搜索 \texttt{git.mergeEditor}，取消勾选即可回退到内联视图。
\end{itemize}

\begin{tipbox}
\textbf{Base 视图的作用}

Base 视图显示的是两个分支分叉前的原始文件状态。当你不确定哪一方的改动更合理时，看看 Base 能帮你理清思路：原来这里最初长什么样。
\end{tipbox}

\subsection{内联冲突解决（Inline Merge）}

对于简单的冲突，VS Code 也支持直接在代码中内联解决。冲突文件会在 Source Control 视图中高亮显示，如图~\ref{fig:merge-inline-actions} 所示。

\vscodeimg{images/34-merge-inline-actions.png}{内联冲突解决操作——冲突代码块上方出现四个操作按钮，可直接选择保留方案}{fig:merge-inline-actions}

当 VS Code 检测到冲突时，会在冲突代码块上方显示四个操作按钮：

\begin{itemize}[leftmargin=2em]
  \item \textbf{Accept Current Change}：保留当前分支的修改
  \item \textbf{Accept Incoming Change}：保留被合并分支的修改
  \item \textbf{Accept Both Changes}：同时保留双方的修改（上下排列）
  \item \textbf{Compare Changes}：打开 Diff 视图，详细对比双方差异
\end{itemize}

点击任意按钮后，VS Code 会自动清理冲突标记，文件状态也会从冲突变为已修改。

\begin{stepbox}
\textbf{动手实验：人为制造并解决冲突}

按以下步骤在本地仓库中体验一次完整的冲突解决流程：

\begin{lstlisting}[style=shell]
# create and switch to new branch
git checkout -b feature
echo "Feature branch line" > README.md
git add README.md && git commit -m "feature update"
# switch back to main and modify same line
git checkout main
echo "Main branch line" > README.md
git add README.md && git commit -m "main update"
# merge feature branch, conflict occurs
git merge feature
# open README.md in VS Code, resolve inline
# then stage and commit
git add README.md && git commit -m "resolve conflict"
\end{lstlisting}
\end{stepbox}

\subsection{AI 辅助解决冲突}

VS Code 新版本（需安装 GitHub Copilot 扩展）支持 AI 辅助合并冲突。当检测到冲突时，Copilot 会分析双方代码的上下文，自动生成一个合理的合并建议。你可以直接采纳建议，或在此基础上手动调整。这能显著减少处理复杂冲突时的认知负担。
...
\noindent\textbf{提示：}\textbf{强烈推荐安装 GitLens}。它提供了：
\begin{itemize}
...
\subsection{GitLens 详细使用指南}

GitLens 是 VS Code 中最流行的 Git 增强扩展，它能在编辑器中直接显示代码的提交历史和作者信息，帮助初学者快速理解代码的演变过程。

\textbf{安装步骤}：按 \texttt{Ctrl+Shift+X} 打开 Extensions 面板，搜索 \texttt{GitLens}，点击 Install。

\textbf{核心功能}：
\begin{itemize}[leftmargin=2em]
  \item \textbf{Inline Blame}：安装后自动在每行代码末尾显示最后修改者和时间。
  \item \textbf{CodeLens}：在函数和类上方显示作者和提交信息。
  \item \texttt{GitLens: Show Commit Graph}：打开提交历史图形视图。
  \item \texttt{GitLens: Open File History}：右键文件 $\to$ 选择此命令查看完整历史。
  \item \texttt{GitLens: Toggle Line Blame} / \texttt{GitLens: Toggle Git CodeLens}：快速开关行内 blame 和 CodeLens 显示。
\end{itemize}

\vscodeimg{images/28-gitlens-blame.png}{GitLens 行内 Blame 与 CodeLens 显示}{fig:ext-gitlens-blame}

\vscodeimg{images/29-gitlens-graph.png}{GitLens 提交图（Commit Graph）}{fig:ext-gitlens-graph}

\textbf{提示}：初学者可以先开启 Inline Blame 和 CodeLens，它们能让你在阅读代码时自然了解修改背景。

\textbf{进阶功能}（GitLens 16+）：

\begin{itemize}[leftmargin=2em]
  \item \textbf{Commit Graph 过滤}：Commit Graph 支持按提交信息、作者、文件、更改类型过滤，输入 \texttt{@me} 只看自己的提交。
  \item \textbf{Home View}：GitLens 16 新增的起始视图，展示当前仓库、检出分支、相关 PR、Launchpad 待审查列表、最近活跃分支/工作树/PR。
  \item \textbf{Launchpad}：优先显示待你审查的内容，集中管理 PR 和审查任务。
  \item \textbf{Start Work}：从 Home View 或命令面板执行 \texttt{GitLens: Start Work} 创建分支并初始化任务。
\end{itemize}
...
\subsection{GitHub Pull Requests 使用指南}

GitHub Pull Requests 扩展让你在 VS Code 中直接查看、创建和审查 Pull Request，无需频繁切换浏览器。

\textbf{安装步骤}：按 \texttt{Ctrl+Shift+X} 打开 Extensions 面板，搜索 \texttt{GitHub Pull Requests and Issues}，点击 Install。安装后需要登录 GitHub：按 \texttt{Ctrl+Shift+P} 打开命令面板，输入 \texttt{GitHub: Sign in} 并按提示完成授权。登录后 Activity Bar 会出现 GitHub 图标。

\textbf{核心功能}：
\begin{itemize}[leftmargin=2em]
  \item \textbf{Activity Bar 中的 GitHub 图标}：点击后查看当前仓库的 PR 列表。
  \item \texttt{GitHub Pull Requests: Create Pull Request}：从当前分支创建 PR。
  \item \textbf{查看 PR 详情}：点击 PR 列表中的条目，可查看 diff 和评论。
  \item \textbf{Checkout PR}：点击 Checkout 按钮切换到 PR 对应的分支进行审查。
\end{itemize}

\vscodeimg{images/30-gh-pr.png}{GitHub Pull Requests 侧边栏中的 PR 列表}{fig:ext-gh-pr}

\textbf{提示}：审查队友的 PR 时，可以直接在代码行上添加评论，所有操作都会同步到 GitHub。

\textbf{进阶功能}：

\begin{itemize}[leftmargin=2em]
  \item \textbf{行内评论（In-editor Commenting）}：审查 PR 时直接在代码行上添加评论，支持单条评论和整体 Review。
  \item \textbf{Review Mode}：点击 Checkout PR 切换到 PR 分支后进入审查模式，出现 Changes in Pull Request 视图可查看 diff 和所有提交；Exit Review Mode 返回原分支。
  \item \textbf{AI 代码审查}：PR 描述页面有 Code Review 按钮，AI 可审查整个 PR 的所有更改（详见第~\ref{sec:ai-git}~节）。
\end{itemize}
...
\subsection{GitHub Issues 使用指南}

GitHub Issues 扩展将 Issue 跟踪功能集成到 VS Code 中，方便你在编码时随时查看和管理任务。

\textbf{安装步骤}：按 \texttt{Ctrl+Shift+X} 打开 Extensions 面板，搜索 \texttt{GitHub Issues}，点击 Install。

\textbf{核心功能}：
\begin{itemize}[leftmargin=2em]
  \item \textbf{侧边栏 GitHub Issues 视图}：显示当前仓库的未关闭 Issues。
  \item \texttt{GitHub Issues: Create Issue from Selection}：选中代码后快速创建 Issue。
  \item \textbf{引用关联}：在提交信息或 PR 描述中输入 \texttt{\#issue-number} 自动关联对应 Issue。
  \item \textbf{创建新 Issue}：点击侧边栏中的按钮直接创建。
\end{itemize}

\textbf{提示}：与 GitHub Pull Requests 扩展配合使用，可以在一个界面中同时管理 Issues 和 PRs。

\subsection{Git Graph 使用指南}

Git Graph 提供直观的图形化分支时间线，让初学者一眼看清项目的分支结构和合并历史。

\textbf{安装步骤}：按 \texttt{Ctrl+Shift+X} 打开 Extensions 面板，搜索 \texttt{Git Graph}，点击 Install。

\textbf{核心功能}：
\begin{itemize}[leftmargin=2em]
  \item \texttt{Git Graph: View Git Graph}：打开彩色分支时间线视图。
  \item \textbf{右键点击提交}：选择 Cherry Pick、Revert、Checkout、Create Branch 或 Merge。
  \item \textbf{右键点击分支}：选择 Checkout、Delete、Merge、Push 或 Rebase。
  \item \textbf{比较提交}：Ctrl+Click 第二个提交，比较任意两个提交的差异。
  \item \textbf{搜索提交}：在图形视图中按 \texttt{Ctrl+F} 搜索提交。
\end{itemize}

\vscodeimg{images/31-git-graph.png}{Git Graph 分支时间线视图}{fig:ext-git-graph}

\textbf{提示}：学习分支与合并时，用 Git Graph 配合第四章内容，能直观理解分支的创建和合并过程。

\subsection{Git History 使用指南}

Git History 专注于查看单个文件或代码行的详细提交历史，界面轻量，适合快速追溯代码变更。

\textbf{安装步骤}：按 \texttt{Ctrl+Shift+X} 打开 Extensions 面板，搜索 \texttt{Git History}，点击 Install。

\textbf{核心功能}：
\begin{itemize}[leftmargin=2em]
  \item \texttt{Git: View File History}：查看当前文件的完整提交历史。
  \item \texttt{Git: View Line History}：查看光标所在行的修改历史。
  \item \texttt{Git: View Branch History}：查看当前分支的提交历史。
  \item \texttt{Git: View Commit History}：查看整个仓库的提交历史。
\end{itemize}

\vscodeimg{images/32-git-history.png}{Git History 文件历史视图}{fig:ext-git-history}

\textbf{提示}：使用前先打开目标文件，再运行命令，否则可能看不到预期的历史记录。

\subsection*{动手实验 6：VS Code中完成完整Git工作流}

花 8 分钟把本章的核心功能串起来走一遍：建分支、编辑、Diff 审查、部分暂存、提交、查看历史图。全程只使用鼠标点击，不需要输入任何命令。

\medskip
\begin{enumerate}[leftmargin=1.5em]
  \item 点击底部状态栏分支名 $\to$「Create new branch…」，输入 \texttt{feature/sc-practice} 并回车。
  \item 在 \texttt{hello.txt} 末尾新增两行内容并保存；在资源管理器中点击文件名打开 Diff 视图，观察行号旁 gutter 上的绿色标记（新增行）（详见第~\ref{sec:diff-editor}~节）。
  \item 点击 Diff 编辑器工具栏的「下一个更改」（Next Change）按钮，在两个修改位置之间跳转；再点击「折叠未更改区域」（Collapse Unchanged Regions）按钮，观察只有差异代码块显示，其余部分被折叠（详见第~\ref{sec:diff-editor}~节）。
  \item 在 Diff 编辑器左侧 gutter 上，点击某行修改旁的 \textbf{Stage} 按钮，把该行直接暂存（部分暂存）；然后回到 Source Control 视图，点击「更改」标题右侧的「+」按钮，把剩余修改全部暂存。
  \item 在提交信息框输入 \texttt{练习部分暂存}。如果已安装 GitHub Copilot 扩展，可点击提交信息框右侧的星标图标，让 AI 自动生成提交信息（没有则手动输入即可）（详见第~\ref{sec:ai-git}~节）。点击「提交」。
...
\noindent\textbf{提示：}\textbf{成功标志}：Diff 编辑器中 gutter 的绿色标记已消失（修改已提交）；提交历史图中出现 \texttt{feature/sc-practice} 分叉；Timeline 视图中 \texttt{hello.txt} 列出了多条提交记录，最近一条是 \texttt{练习部分暂存}。
\medskip
...
\noindent\textbf{提示：}\textbf{成功标志}：GitHub 上 \texttt{main} 分支包含 \texttt{e2e.md}，且本地 \texttt{feature/e2e-flow} 分支已删除，整个 Feature Branch 流程闭环完成。
